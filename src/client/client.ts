import { API_ENDPOINT, MAX_UPLOADS } from '../server/constants';
...
async function handleConversion(selectedFiles: FileList) {
  if (selectedFiles.length > MAX_UPLOADS) {
    const msg = `ERROR: UPLOAD LIMIT EXCEEDED!<br>The number of files to be uploaded should not exceed ${MAX_UPLOADS}.`;
    showAlert(msg, 'error');
    isLoading = false;
